...
class AzureSasStorage(StorageBase):
    """Azure SAS存储实现"""
    
    def __init__(self, container_url: str, sas_token: str):
        """初始化Azure SAS存储"""
        self.container_url = container_url
        self.sas_token = sas_token
        self.default_bucket_name = APP_NAME.lower().replace("_", "-")

        if not self.container_url or not self.sas_token:
            raise ValueError("Azure SAS存储需要配置container_url和sas_token")
    ...
    async def get(self, file_index: str, bucket_name: Optional[str] = None) -> Optional[BinaryIO]:
        ...
        try:
            ...
            for attempt in range(ATTEMPT_TIME):
                try:
                    blob_client = await asyncio.to_thread(self.conn.get_blob_client, file_index)
                    download_stream = await asyncio.to_thread(blob_client.download_blob)
                    return BytesIO(await asyncio.to_thread(download_stream.readall))
                except Exception as e:
                    if attempt < ATTEMPT_TIME - 1 and self._should_retry(e):
    ...
    async def delete(self, file_index: str, bucket_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            for attempt in range(ATTEMPT_TIME):
                try:
                    await asyncio.to_thread(self.conn.delete_blob, file_index)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logging.error(f"Azure SAS健康检查失败: {e}")
            return False
    ...
    async def close(self):
        ...
        try:
            if self.conn:
                del self.conn
                self.conn = None
                logging.info("Azure SAS连接已关闭")
        except Exception as e:
            logging.error(f"关闭Azure SAS连接失败: {e}")
        ...
        if self._should_check_health():
            try:
                if not await self._health_check():
                    logging.warning("Azure SAS连接不健康，重新连接")
                    async with self._connection_lock:
                        await self._connect()  # 重新连接
            except Exception as e:
                logging.warning(f"Azure SAS连接健康检查失败: {e}，重新连接")
                async with self._connection_lock:
                    await self._connect()  # 重新连接
        ...
        for attempt in range(ATTEMPT_TIME):   
            try:
                ...
                if await self._health_check():
                    logging.info(f"Connected to Azure SAS {self.container_url}")
                    return  # 连接成功，直接返回
                else:
                    logging.warning(f"Azure SAS {self.container_url} 连接失败，等待重试...")
            
            except asyncio.CancelledError:
                logging.error(f"Azure SAS连接被取消: {self.container_url}")
                raise
            except Exception as e:
                logging.warning(f"Azure SAS {self.container_url} 连接异常: {e}")

            if attempt < ATTEMPT_TIME - 1:  # 不是最后一次尝试
                await asyncio.sleep(RETRY_DELAY)
        
        # 如果所有重试都失败了
        msg = f"Azure SAS {self.container_url} 连接失败，已尝试 {ATTEMPT_TIME} 次"
        logging.error(msg)
    ...
    async def _health_check(self) -> bool:
        ...
        try:
            if self.conn:
                # 尝试获取容器属性来验证连接
                await asyncio.to_thread(self.conn.get_container_properties)
                return True
            return False
        except Exception as e:
            logging.error(f"Azure SAS健康检查失败: {e}")
